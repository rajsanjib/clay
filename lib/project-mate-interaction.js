...
function attachMateInteraction(ctx) {
  ...
  function handleMention(ws, msg) {
    ...
    // Agent mention: route through the current session's Agent tool.
    // The message is injected as a regular user message so the active agent
    // (e.g. narada) dispatches it to the named sub-agent.
    if (msg.mateId && msg.mateId.indexOf("agent:") === 0) {
      var agentName = msg.mateId.slice(6);
      var agentText = "@" + agentName + " " + (msg.text || "");

      // Record in history as a user message with mention metadata
      var historyEntry = {
        type: "user_message",
        text: agentText,
      ...
      sm.appendToSessionFile(session, historyEntry);

      // Broadcast the user message to all clients so it appears in chat
      sendToSession(session.localId, {
        type: "user_message",
        text: agentText,
        _ts: historyEntry._ts,
      });

      sm.broadcastSessionList();

      // Dispatch to the coding agent via SDK
      if (!session.isProcessing) {
        session.isProcessing = true;
        session._queryStartTs = Date.now();
        sendToSession(session.localId, { type: "status", status: "processing" });
        console.log("[agent-mention] startQuery, localId=" + session.localId + " vendor=" + session.vendor + " agent=" + agentName);
        try {
          sdk.startQuery(session, agentText, null, null);
        } catch (e) {
          console.error("[agent-mention] startQuery failed:", e.message || e);
          session.isProcessing = false;
          sendToSession(session.localId, { type: "error", text: "Failed to dispatch agent @" + agentName + ": " + (e.message || e) });
        }
      } else {
        console.log("[agent-mention] pushMessage, localId=" + session.localId + " agent=" + agentName);
        sdk.pushMessage(session, agentText, null);
      }
