...
function attachMateInteraction(ctx) {
  ...
  function handleMention(ws, msg) {
    ...
    // The message is injected into the conversation and the active agent
    ...
    if (msg.mateId && msg.mateId.indexOf("agent:") === 0) {
      ...
      var agentMsg = { type: "user", message: { role: "user", content: [{ type: "text", text: agentText }] } };
      // Send as user acknowledgment
      ...
      sendToSession(session.localId, {
        type: "mention_delta",
        mateId: msg.mateId,
        delta: "Dispatching @" + agentName + "...",
      });
      sendToSession(session.localId, {
        type: "mention_done",
        mateId: msg.mateId,
        text: "Dispatching @" + agentName + "...",
      });
      // Record in history before dispatching to SDK
      var historyEntry = {
        type: "mention_user",
        text: msg.text || "",
        mateId: msg.mateId,
      ...
      // Broadcast the mention to the UI
      ...
      if (!session.isProcessing) {
        ...
        console.log("[agent-mention] startQuery, localId=" + session.localId + " agent=" + agentName);
