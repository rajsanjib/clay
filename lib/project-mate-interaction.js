...
function attachMateInteraction(ctx) {
  ...
  function handleMention(ws, msg) {
    ...
    if (msg.mateId && msg.mateId.indexOf("agent:") === 0) {
      ...
      // Now queue the message to the coding agent session
      if (session._queueUserMessage) {
        session._queueUserMessage(agentText);
      }
      // Record in history
      ...
      sm.appendToSessionFile(session, session.history[session.history.length - 1]);
      return;
    }
